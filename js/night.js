...
export function createNight(deps) {
  ...
  const list = [];
  const bodyCache = new Map(); // 9 zumbis e 5 fantasmas = 2 geometrias, não 14
  function makeZombie() {
    ...
    /* PERF: o zumbi não articula nada (só cambaleia o grupo inteiro), então as
       9 peças viram 4 malhas — uma por (material, castShadow). Sem ossos:
       nenhuma parte gira sozinha. Ver fuseBody() em js/meshutils.js. */
    fuseBody(g, { cache: bodyCache, cacheKey: 'zumbi' });
    return g;
  }
  function makeGhost() {
    const g = new THREE.Group();
    ...
    // véu (corpo + cauda, mesmo material translúcido) num só; olhos noutro
    const { meshes } = fuseBody(g, { cache: bodyCache, cacheKey: 'fantasma' });
    return { g, veu: meshes.find(m => m.material === gMat) || meshes[0] };
  }
  function makeCreature(ghost) {
    const feito = ghost ? makeGhost() : { g: makeZombie(), veu: null };
    const g = feito.g;
    g.visible = false;
    scene.add(g);
    const c = {
      ghost, group: g, veu: feito.veu, // o véu pulsa a opacidade (era children[0])
  ...
  function update(dt, t) {
    ...
    for (const c of list) {
      ...
      if (c.ghost) {
        ...
        c.veu.material.opacity = 0.2 + Math.sin(t * 3 + c.phase) * 0.1;
      } else {
