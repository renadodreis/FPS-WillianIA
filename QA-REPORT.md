...
| 20 | 🔴 crítica | rede | Dada uma queda de conexão, quando o socket.io reconectava, então o servidor criava um id novo → **avatar duplicado** e identidade quebrada | `reconnect` → `location.reload()` (reentra limpo como espectador). **Atualizado em 2026-08-07:** no LOBBY, com a mesma seed, o cliente ADOTA o `init` novo em vez de recarregar (`multiplayer-client.js`) e re-reivindica o posto de anfitrião; o reload segue valendo em partida. | manual + `test/lobby-reconnect-host.test.js` |
